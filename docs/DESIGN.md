...
- **Catalog dedupe:** One entry per game base title. Regional variants collapse with priority USA → World → USA/Europe combo → Europe → other countries → Japan/Asia. Among ties, the variant with the most artwork (box art + title screen + snap) wins; lowest revision wins after that. Beta and Proto releases are excluded.
- **Search display:** Results show the stripped base title (no region/revision suffixes). `libretroName` is kept for artwork URL lookup.
