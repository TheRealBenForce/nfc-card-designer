...
> **Proposed change:** See [GitHub Pages + libretro GitHub raw URLs](#github-pages--libretro-github-raw-urls-zero-image-hosting) in the backlog. Artwork from GitHub raw URLs; game names in bundled `game-catalog.json` built by `build-game-catalog.mjs` at deploy time.
...
| `npm run deploy` | **Keep** for AWS; add `build-game-catalog`; remove `fetch-images`, `sync-image-manifest`, `sync-s3-sample-images` |
