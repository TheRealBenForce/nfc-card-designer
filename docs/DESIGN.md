...
- **Catalog dedupe:** One entry per game base title. Regional variants collapse with priority USA → World → USA/Europe combo → Europe → other countries → Japan/Asia. Among ties, the variant with the most artwork (box art + title screen + snap) wins; lowest revision wins after that. Multi-disc releases collapse to one entry (prefer Disc 1). Beta, Proto, Bootleg, Homebrew, translation patches, SymbolicLink, and path-junk entries are excluded.
- **Search display:** Results show a cleaned base title — trailing `(...)` / `[...]` metadata, version tokens (`v1.400`), region/revision/disc/publisher/serial/dump flags are stripped. Spaced underscores between alternate titles become dashes. In-title parentheses that are part of the game name are kept. `libretroName` remains the exact GitHub filename stem for artwork URLs.
- **Saved collections:** Cards persist `libretroName` as the canonical artwork key. `gameName` is derived for display on load/save, so catalog filter/display changes do not break artwork for existing collections.
