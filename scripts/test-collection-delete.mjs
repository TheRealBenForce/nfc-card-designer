...
async function main() {
  ...
  try {
    ...
    if ((await page.locator(".collection-card").count()) !== 1) {
      throw new Error("Expected one collection card after add");
    }
    ...
    await page.locator(".collection-card__select-btn").first().click();
